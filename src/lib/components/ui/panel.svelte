...
{#if animation.showPanel && showPanel}
  {#key itemKey}
    ...
    <aside
      use:slideIn
      class="fixed top-0 right-0 z-[70] h-dvh bg-background shadow-2xl transition-[width,border-radius,border-left] duration-300 ease-out"
      style="width: {animation.panelWidth}; border-top-left-radius: {animation.borderRadius}; border-left: {animation.borderLeft}"
    >
      <div class="sticky top-0 z-10 border-b bg-background/80 backdrop-blur">
        <div class="flex min-h-14 items-center justify-between gap-3 px-4 py-3">
          <div class="flex min-w-0 items-center gap-2">
            <div class="min-w-0 truncate text-sm font-semibold">
              {title}
              {#if subtitle}
                <span class="ml-2 text-xs font-normal text-muted-foreground">{subtitle}</span>
              {/if}
            </div>
