<script lang="ts">
  ...
  const props: NodeViewProps = $props();
  const uploadOptions = $derived(props.editor.storage.imageUpload);
...
<NodeViewWrapper class="my-2 w-full rounded-lg border bg-muted/50 p-4">
  {#if uploadState.mode === 'uploading'}
