<script lang="ts">
	...
	import { FOCUS_PARAM, detailPath } from './route';
	...
	let formSelections = $state<Record<string, number>>({});
let formIndex = $derived.by(() => (focusId ? (formSelections[focusId] ?? 0) : 0));
	let activeForm = $derived(focusedPokemon?.forms[formIndex] ?? null);

	function selectForm(index: number) {
		if (!focusId) return;
		formSelections = { ...formSelections, [focusId]: index };
	}
