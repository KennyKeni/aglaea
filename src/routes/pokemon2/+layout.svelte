<script lang="ts">
	...
	import { FOCUS_PARAM, ROUTE_BASE, detailPath, getDetailIdFromPath, listPath } from './route';
	...
	onNavigate((navigation) => {
		...
		const fromId = getDetailIdFromPath(fromPath);
		const toId = getDetailIdFromPath(toPath);
		const toFocus = navigation.to?.url.searchParams.get(FOCUS_PARAM);

		// Skip view transition for expand/collapse (same pokemon, just changing view)
		if ((fromId && toFocus === fromId) || (toId && fromPath === ROUTE_BASE)) {
			return;
		}
	...
	let focusId = $derived(page.url.searchParams.get(FOCUS_PARAM));
	...
	function openPeek(mon: Pokemon) {
		goto(listPath(mon.id), { noScroll: true });
		preloadData(detailPath(mon.id));
	}

	function expand() {
		if (!focusId) return;
		goto(detailPath(focusId), { noScroll: true });
	}

	function collapse() {
		if (!(isDetailRoute || isNavigatingToDetail)) return;
		const currentId = navigating?.to?.params?.id ?? page.params.id ?? focusId;
		if (!currentId) return;
		goto(listPath(currentId), { noScroll: true });
	}

	function close() {
		goto(listPath(), { noScroll: true });
	}

	const slideIn: Attachment<HTMLElement> = (node) => {
